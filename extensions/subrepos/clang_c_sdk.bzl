...
#     )
    ...
    if rctx.attr.is_distro:
        rctx.symlink("{root}/include/clang-c".format(
            root=rctx.attr.llvm_root), # wsroot),
                     "include/clang-c")
    else:
        rctx.symlink("{root}/../llvm/include/clang-c".format(
            root=rctx.attr.llvm_root), # wsroot),
                     "include/clang-c")
...
"""
    ...
    ## end of _clang_c_sdk repo rule

############
repo_clang_c_sdk = repository_rule(
    implementation = _clang_c_sdk_impl,
    local = True,
    attrs = {
        "version": attr.string(),
